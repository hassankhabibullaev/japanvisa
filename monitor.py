    ...
    if ENABLE_TELEGRAM and TG_TOKEN and TG_CHAT:
        try:
            ...
            status, _ = http_get(
                ...
                % (TG_TOKEN, urllib.parse.urlencode({"chat_id": TG_CHAT, "text": "🚨 " + text}))
            )
            log("notify telegram: HTTP %d (%.1fs)" % (status, time.time() - t0))
            ok_any = ok_any or status == 200
        except Exception as e:
            log("notify telegram FAILED: %s" % e)
...
def main():
    ...
    while True:
        ...
        if new:
            log("NEW OPENING(S): %s — alerting (cycle took %.1fs)" % (sorted(new), time.time() - t0))
            notify(open_dates)
        if open_dates != state:
            state = open_dates
            save_state(state)
        log("checked in %.1fs — open: %s" % (time.time() - t0, sorted(open_dates) or "none"))
